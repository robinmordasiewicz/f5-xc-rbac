...
| XC_CERT | Secret file | `xc-cert` | PEM certificate file |
| XC_CERT_KEY | Secret file | `xc-cert-key` | PEM private key file |

**Setup**: Upload your `cert.pem` and `key.pem` files generated by the setup script (see [Getting Started](../get-started.md#setup-credentials)).
